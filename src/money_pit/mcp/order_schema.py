...
ALPACA_ORDER_SCHEMA_STUB_SENTINEL: str = "x_stub"
...
class AlpacaOrderSchemaNotPinnedError(AlpacaOrderSchemaError):
    """Raised when the schema file present is still the unpinned stub (sentinel present) — distinct from absent or malformed."""


def load_order_schema(path: Path = ALPACA_ORDER_SCHEMA_PATH) -> dict[str, object]:
    """Load the pinned Alpaca order JSON schema, failing closed if absent, malformed, or still the unpinned stub."""
    if not path.exists():
    ...
    schema: dict[str, object] = cast("dict[str, object]", raw)
    if schema.get(ALPACA_ORDER_SCHEMA_STUB_SENTINEL):
        raise AlpacaOrderSchemaNotPinnedError(
            f"Alpaca order schema at {path} is still the committed stub"
            f" (sentinel {ALPACA_ORDER_SCHEMA_STUB_SENTINEL!r} present)."
            + " Pin the real schema from the live Alpaca MCP server."
        )
    return schema
