...
│   ├── __init__.py
│   ├── enums.py             # Every canonical enum: SignalTier, ClaimCategory, ActionType, ExecutionPhase, TerminalState, …
...
│   ├── validator.py         # A5: manifest parse, jsonschema checks, action_type→tool routing, three-file write
...
│   ├── regime.py            # Regime decision table: four indicators → RegimeTag (UNCERTAIN on missing/conflict)
...
└── mcp/                     # MCP client configuration; runs inside the pipeline process
    ├── __init__.py
    ├── clients.py           # AlpacaReadDeps, AlpacaWriteDeps dep types + factory fns; injection-ready for Pydantic AI
    └── manifest.py          # Introspects registered MCP servers at runtime → tool manifest consumed by A5
...
**`compute/` is the highest-value test target.** Every function here is pure or near-pure. The architecture and pipeline contracts both enumerate specific computations that must not be in LLM (`pipeline_contracts.md §9`); `compute/` is the enforcement mechanism. `compute/aggregation.py` owns claim union, run-global re-ID, and tier max — deterministic functions that belong here, not inside the `pipeline/aggregator.py` node.
...
3. **`compute/execution_params.py` depends on the Alpaca MCP order tool's OpenAPI schema** (an external artifact). Needs a pinned snapshot or startup introspection before A5 can be built and verified; not a structural flaw, but a deployment prerequisite.
