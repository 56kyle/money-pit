...
@pytest.fixture(scope="session")
def stub_free_order_schema_path(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """Path to a sentinel-free copy of the committed Alpaca order schema.

    Derived from the committed alpaca_order_schema.json with the stub sentinel key
    dropped, so it stays in lockstep with the real stub's field shape. Injecting this
    via the order_schema_path / schema_path seams opts a run out of the fail-closed
    unpinned-stub gate. When the real schema is pinned (sentinel removed upstream),
    the pop is a no-op and this fixture keeps working — the tripwire in
    test_order_schema.py is what then signals the opt-ins can be removed.
    """
    schema: dict[str, object] = json.loads(ALPACA_ORDER_SCHEMA_PATH.read_text(encoding="utf-8"))
    _ = schema.pop(ALPACA_ORDER_SCHEMA_STUB_SENTINEL, None)
    path: Path = tmp_path_factory.mktemp("stub_free_order_schema") / "alpaca_order_schema.json"
