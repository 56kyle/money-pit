...
def test_run_pipeline_with_missing_capital_dep(tmp_path: Path, missing_dep: str) -> None:
    overrides = phase4_overrides()
    setattr(overrides, missing_dep, None)
    with pytest.raises(MissingPipelineDependencyError):
        _ = run_pipeline(signals_dir=_SIGNALS_DIR, run_dir=tmp_path / "run", overrides=overrides)


def test_run_pipeline_with_no_overrides(tmp_path: Path) -> None:
    with pytest.raises(MissingPipelineDependencyError):
        _ = run_pipeline(signals_dir=_SIGNALS_DIR, run_dir=tmp_path / "run")


def test_run_pipeline_with_full_phase4_overrides(tmp_path: Path) -> None:
    final_state = run_pipeline(signals_dir=_SIGNALS_DIR, run_dir=tmp_path / "run", overrides=phase4_overrides())
