...
An A3 job fingerprint uses the complete immutable A2 task premise associated with its discovery origins, not the currently pending subset. Queue status is lifecycle state and cannot change semantic job identity. A3 planner follow-up tasks advance the existing job rather than creating a new job. If an exact cycle-zero premise task was already completed before a retry claims the job, the retry treats it as consumed work and does not rematerialize or repay for it.
