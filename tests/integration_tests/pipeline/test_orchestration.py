...
def test_run_pipeline_with_missing_capital_dep(
    tmp_path: Path, pipeline_signals_dir: Path, missing_dep: str
) -> None:
...
def test_run_pipeline_with_full_phase4_overrides(tmp_path: Path, pipeline_signals_dir: Path) -> None:
    final_state = run_pipeline(
        signals_dir=pipeline_signals_dir, run_dir=tmp_path / "run", overrides=phase4_overrides()
    )
