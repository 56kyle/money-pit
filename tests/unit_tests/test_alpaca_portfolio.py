...
The yfinance network fetch is isolated in _fetch_ticker_info; the mappers under test
(_as_float, _resolve_sector, _factor_metrics_from_info, _is_etf, _to_position) are pure and take
plain dicts / a pre-fetched `.info` mapping, so no monkeypatching is needed. _to_position maps one
alpaca-py raw position onto our frozen Position and fails closed on non-equity assets. The alpaca-py
TradingClient calls inside make_alpaca_portfolio_fetcher are live-only and pinned by the live tier.
Assertions target the NonEquityPositionError TYPE and concrete field/model values.
"""
...
from money_pit.alpaca_portfolio import _as_float
from money_pit.alpaca_portfolio import _factor_metrics_from_info
from money_pit.alpaca_portfolio import _is_etf
from money_pit.alpaca_portfolio import _resolve_sector
...
def test__as_float_with_valid(value: object, expected: float) -> None:
...
def test__as_float_with_invalid(value: object) -> None:
...
def test__resolve_sector(info: dict[str, object], expected: str) -> None:
    assert _resolve_sector(info) == expected
...
def test__is_etf(info: dict[str, object], expected: bool) -> None:
    assert _is_etf(info) is expected
